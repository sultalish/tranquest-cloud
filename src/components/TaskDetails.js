...
const TaskDetails = () =>{
    ...
    const markComplete = (event, id) => {
      ...
      db.collection('users').doc(auth.currentUser.uid).update("lastTimeCompletedTask", new Date());

      // Update badge progress
      db.collection('users').doc(auth.currentUser.uid).collection('badges').get().then(function(querySnapshot) {
          querySnapshot.forEach(async function(badge) {
              let badge_ref = db.collection('users').doc(auth.currentUser.uid).collection('badges').doc(badge.id);
              badge_ref.update("progress", firebase.firestore.FieldValue.increment(1));
              let doc = await badge_ref.get();
              let badge_data = doc.data();
              if (!badge_data.dateAchieved && badge_data.progress === badge_data.sentinel) {
                console.log(`Unlocked achievement: ${badge_data.name}`);
                badge_ref.update("dateAchieved", new Date());
              };
          });
      });
